...
class LagMaximFileHandler(FileHandler):
    ...
    def __init__(self, upload_file):
        ...
        self.assay_name = 'LAg-Maxim'

    def parse(self):
        from assay.models import LagMaximResultRow
        rows_inserted = 0
        rows_failed = 0
        ...
        for row_num in range(self.num_rows):
            try:
                if row_num >= 1:
                    ...
                    lag_row = LagMaximResultRow.objects.create(specimen_label=row_dict['specimen_label'],
    ...
    def validate(self, panel_id):
        ...
        from assay.models import LagMaximResultRow, LagMaximResult, PanelMembership

        rows_validated = 0
        rows_failed = 0

        for lag_row in LagMaximResultRow.objects.filter(fileinfo=self.upload_file, state='pending'):
            try:
                error_msg = ''
    ...
    def process(self, panel_id, assay_run):
        ...
        from assay.models import LagMaximResultRow, LagMaximResult, AssayResult, PanelMembership
        ...
        for lag_row in LagMaximResultRow.objects.filter(fileinfo=self.upload_file, state='validated'):
            try:
                warning_msg = ''
                ...
                with transaction.atomic():
                    ...
                    lag_result = LagMaximResult.objects.create(specimen=specimen,
                                                               assay=assay,

...
class Command(BaseCommand):
    ...
    def _handle_lag_sedia(self, assay_run):
        ...
        with transaction.atomic():
            for specimen_id in specimen_ids:
                ...
                if lag_result.specimen and lag_result.specimen.is_artificial:
    ...
    def _handle_idev3(self, assay_run):
        ...
        with transaction.atomic():
            ...
            for specimen_id in specimen_ids:
                ...
                result_count = spec_results.count()
                
                for idev3_result in spec_results:
                    if result_count > 1:
    ...
    def _handle_biorad_avidity_glasgow(self, assay_run):
        glasgow_calc = BioRadCalculation(assay_run, BioRadAvidityGlasgowResult,
                                         ['treated_urea_OD', 'untreated_buffer_OD'])
    ...
    def _handle_bioplex_cdc(self, assay_run):
        ...
        with transaction.atomic():
            for specimen_id in specimen_ids:
                ...
                if luminex_result.specimen.is_artificial:
                    warning_msg += "Artificial aliquot created\n"

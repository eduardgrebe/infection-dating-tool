...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        with transaction.atomic():
            ...
            on_interruption  = Visit.objects.filter(Q(subject__isnull=False,
                                                      subject__art_initiation_date__isnull=False,
                                                      visit_date__gt=F('subject__art_interruption_date')))
            on_interruption = on_interruption.filter(Q(visit_date__lt=F('subject__art_resumption_date')) |
            ...
            on_treatment = Visit.objects.filter(on_treatment=True)
            not_on_treatment = Visit.objects.filter(on_treatment=False)
            not_on_treatment.update(first_treatment=None)

            on_first_treatment = on_treatment.filter(Q(subject__art_interruption_date__isnull=True) |

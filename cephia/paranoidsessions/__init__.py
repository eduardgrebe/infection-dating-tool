...
class NonceStream(object):
    ...
    def __init__(self):
        ...
        self.state = hashlib.md5("%s%s" % seed).hexdigest()
    ...
    def nonces(self):
        ...
        while True:
            yield state
            state = hashlib.md5(state + settings.SECRET_KEY).hexdigest()

    def increment(self):
        """Increment the nonce stream, discarding initial nonce."""
        state = self.state
        state = self.state
        self.state = hashlib.md5(state + settings.SECRET_KEY).hexdigest()
        return self.state
    ...
    def check_request(self,request):
        ...
        if request.is_secure():
            if self.secure_key is None:
                seed = (randrange(0,MAX_NONCE_SEED),settings.SECRET_KEY)
                self.secure_key = hashlib.md5("%s%s" % seed).hexdigest()
